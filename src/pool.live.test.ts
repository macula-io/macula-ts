...
import { poolSubscribeUntilDelivered, publishUntilDelivered } from "../test/live_registration.js";
...
describe.skipIf(!process.env.MACULA_TS_LIVE)("Pool (live fleet)", () => {
  ...
  it("subscribe() replays onto a fresh link after its connection is kicked -- events keep arriving, not just the status flag recovering", async () => {
    ...
    try {
      ...
      const subscribed = await poolSubscribeUntilDelivered({
        label: "pool subscribe",
        pool,
        realm: undefined,
        topic,
        handler: (evt) => received.push(evt.payload),
        identity: topicId,
        publish: () => pubSession.publish(topic, { seq: 1 }, {}),
        delivered: sawSeq(1),
      });
      const unsubscribe = subscribed.stop;
